...
# make big file
fallocate -l 6G friendlycore-lite-focal-kernel6-arm64/rootfs.img

# calc image size
IMG_SIZE=`du -s -B 1 friendlycore-lite-focal-kernel6-arm64/rootfs.img | cut -f1`

# re-gen parameter.txt
./tools/generate-partmap-txt.sh ${IMG_SIZE} friendlycore-lite-focal-kernel6-arm64

./mk-sd-image.sh friendlycore-lite-focal-kernel6-arm64
sudo ./mk-emmc-image.sh friendlycore-lite-focal-kernel6-arm64
